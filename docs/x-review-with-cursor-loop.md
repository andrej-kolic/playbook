...
| Default model `grok-4.7-high-fast`, never `auto` | Swapped in 2026-09-22 from `cursor-grok-4.6-high-fast` as a straight version bump (vendor dropped the `cursor-` prefix for this generation) — **untested, zero dogfood runs**, not yet backed by the two-dogfood-run bar above; see "Still open". `cursor-grok-4.6-high-fast`'s own selection came from a real cost-profile calibration, not a guessed plateau, and that data stays attached to it, not to 4.7. `auto` can route to a model from the host's own vendor/family and remove the independent reviewer, regardless of which model is pinned. Do not assert a measured false-positive ranking vs other models; Grok's comparison log is gone. |
...
- `grok-4.7-high-fast` as default — **zero dogfood runs**. Needs two independent agreeing runs per the policy above before it's trusted the way the 4.6 baseline was.
