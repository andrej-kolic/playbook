...
3. Confirm Cursor can actually see `x-code-review-standards` — check `.cursor/skills/x-code-review-standards/SKILL.md` (project) or `~/.cursor/skills/x-code-review-standards/SKILL.md` (global). Either is sufficient: tested directly with only the global copy present (project copy removed), `agent --workspace <repo-root>` still resolved `/x-code-review-standards` and produced standards-formatted output, so `--workspace` isn't project-only (2026-09-10). If neither exists, tell the user to run `pnpm generate:user` (or the project's own `pnpm generate`) first and stop — otherwise Step 3's failure looks like "Cursor produced no findings" when the real cause is "Cursor doesn't have the skill to invoke."
4. Refuse to run on `main`/the base branch itself — **unless `--base worktree`**, which is allowed on any branch.
5. Compute the file list yourself, unrestricted — Cursor's `agent` can't run git under `--mode plan` (or `--mode ask`; both reject `git diff`/`git status` even though neither mutates anything — tested directly, don't assume otherwise). `git diff --name-only` alone misses **untracked new files** (found by dogfooding, 2026-09-10 — 3 of 8 real changed paths were silently dropped) — always union it with `git ls-files --others --exclude-standard`, deduped. It also includes **deleted files** by default, which sends paths that no longer exist to Cursor — always pass `--diff-filter=d` to exclude them. `--base <ref>`: `mergeBase=$(git merge-base <ref> HEAD)`, files = `git diff --name-only --diff-filter=d "$mergeBase"` ∪ untracked (includes uncommitted changes automatically). `--base worktree`: files = `git diff --name-only --diff-filter=d HEAD` ∪ untracked.
6. Empty file list → tell the user there's nothing to review, stop.
7. State the resolved `--base`/`--model`/`--mode` before calling Cursor — on a host where `ARGUMENTS` injection is unverified, this surfaces a silent wrong-default (e.g. reviewing `main` when the user meant `worktree`) before the call happens, not after (dogfooding finding, 2026-09-10). Break the file count down by source in the same line — `N from diff, M untracked` — and name the untracked ones: step 5's untracked union pulls in every untracked file in the repo, not just ones related to this change, so an unrelated file joining the list is visible before it's sent to Cursor, not discovered afterward in its findings.
