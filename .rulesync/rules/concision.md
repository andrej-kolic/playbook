...
<!-- playbook:concision v6 (2026-09-23) — anything checkable carries its pointer -->
...
6. **Name the thing.** So the reader knows it at a glance: a page by its address or visible title, a UI element by its look and position, a file by its path — never by a term from a source the reader hasn't read (the code, a design doc) or a label you coined. Anything the reader might want to check carries its pointer — a change, a fact, a claim, in a list or in prose: whatever lets them see or check it in one step without searching — the running page with its section anchor, the PR or comment link, a `file:line`, the before → after value, or a one-line example. Pick whichever is quickest for this reader, and one that still works where and when they read it: a local URL only in the live session, never in an issue, PR or commit, which outlive the server and are read by people without it — there, a commit, a permalink or a deployed page. If they would have to go looking, the sentence isn't finished. No intensifiers, no hedges, no "it's worth noting", no restating what you just did before doing the next part.
